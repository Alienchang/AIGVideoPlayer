...
@interface AIGCacheConfiguration () <NSCoding ,NSSecureCoding>
...
+ (instancetype)configurationWithFilePath:(NSString *)filePath {
    ...
    if (@available(iOS 11.0, *)) {
        ...
        configuration = [NSKeyedUnarchiver unarchivedObjectOfClasses:[NSSet setWithArray:@[
                                                                                           [AIGCacheConfiguration class],
                                                                                           [NSArray class],
                                                                                           [NSValue class],
                                                                                           [AIGContentInfo class],
                                                                                           [NSURL class]
    ...
    if (!configuration) {
        configuration = [[AIGCacheConfiguration alloc] init];
...
- (id)copyWithZone:(nullable NSZone *)zone {
    AIGCacheConfiguration *configuration = [[AIGCacheConfiguration allocWithZone:zone] init];
...
@implementation AIGCacheConfiguration (AIGConvenient)

+ (BOOL)createAndSaveDownloadedConfigurationForURL:(NSURL *)url error:(NSError **)error {
    NSString *filePath = [AIGCacheManager cachedFilePathForURL:url];
    ...
    AIGContentInfo *contentInfo = [AIGContentInfo new];
