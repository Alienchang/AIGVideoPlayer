...
- (void)mediaCacheDidChanged:(NSNotification *)notification {
    if ([self.delegate respondsToSelector:@selector(cacheProgress:)]) {
        ...
        AIGCacheConfiguration *configuration = userInfo[AIGCacheConfigurationKey];
...
//            [self.player ];
            ...
            _currentFps = 0;
            /// 播放失败
            self.playerStatus = AIG_ENUM_PLAYER_FAILURE;
        } else {
            /// 未知状态
...
- (void)addNotification {
    /// 添加视频缓存进度
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(mediaCacheDidChanged:) name:AIGCacheManagerDidUpdateCacheNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(mediaCacheFinished:) name:AIGCacheManagerDidFinishCacheNotification object:nil];
...
- (void)playbackFinished:(NSNotification *)notification {
    if ([notification.object isMemberOfClass:[AVPlayerItem class]] && self.playerItem == notification.object) {
        if (self.loop) {
            ...
            self.playerStatus = AIG_ENUM_PLAYER_FINISHED;
        }
    }
}
...
- (void)setContentMode:(AIGVideoFillMode)contentMode {
    ...
    switch (contentMode) {
        case AIGVideoGravityResizeAspect:
            [self.playerLayer setVideoGravity:AVLayerVideoGravityResizeAspect];
            break;
        case AIGVideoGravityResizeAspectFill:
            [self.playerLayer setVideoGravity:AVLayerVideoGravityResizeAspectFill];
            break;
        case AIGVideoGravityResize:
...
- (void)preload:(NSString *)url
    ...
    if (self.mediaDownloaders.count >= self.maxPreloadLimit) {
        return;
    }
...
- (void)pause {
    self.playerStatus = AIG_ENUM_PLAYER_PAUSE;
    [self.player pause];
}

- (void)resume {
    self.playerStatus = AIG_ENUM_PLAYER_PLAYING;
    [self.player play];
}
...
- (void)seek:(NSTimeInterval)time {
    ...
    [self.player seekToTime:CMTimeMake(time * 600, 600) toleranceBefore:CMTimeMake(1, 600) toleranceAfter:CMTimeMake(1, 600) completionHandler:^(BOOL finished) {
        ...
        strongSelf.playerStatus = AIG_ENUM_PLAYER_SEEK_COMPLETED;
        [strongSelf resume];
        [self setSeeking:NO];
    }];
}
...
- (void)setPlayerStatus:(AIG_ENUM_PLAYER_STATUS)playerStatus {
