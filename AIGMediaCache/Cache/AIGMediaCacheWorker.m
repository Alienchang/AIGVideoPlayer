...
@interface AIGMediaCacheWorker ()

@property (nonatomic, strong) NSFileHandle *readFileHandle;
@property (nonatomic, strong) NSFileHandle *writeFileHandle;
@property (nonatomic, strong, readwrite) NSError *setupError;
@property (nonatomic, copy) NSString *filePath;
@property (nonatomic, strong) AIGCacheConfiguration *internalCacheConfiguration;
...
@implementation AIGMediaCacheWorker
...
- (instancetype)initWithURL:(NSURL *)url {
    self = [super init];
    if (self) {
        NSString *path = [AIGCacheManager cachedFilePathForURL:url];
        ...
        if (!error) {
            ...
            if (!error) {
                ...
                _internalCacheConfiguration = [AIGCacheConfiguration configurationWithFilePath:path];
...
- (AIGCacheConfiguration *)cacheConfiguration {
...
- (NSArray<AIGCacheAction *> *)cachedDataActionsForRange:(NSRange)range {
    ...
    [cachedFragments enumerateObjectsUsingBlock:^(NSValue * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (intersectionRange.length > 0) {
            ...
            for (NSInteger i = 0; i <= package; i++) {
                AIGCacheAction *action = [AIGCacheAction new];
                action.actionType = AIGCacheAtionTypeLocal;
    ...
    if (actions.count == 0) {
        AIGCacheAction *action = [AIGCacheAction new];
        action.actionType = AIGCacheAtionTypeRemote;
        action.range = range;
        [actions addObject:action];
    } else {
        // Add remote fragments
        NSMutableArray *localRemoteActions = [NSMutableArray array];
        [actions enumerateObjectsUsingBlock:^(AIGCacheAction * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (idx == 0) {
                ...
                AIGCacheAction *lastAction = [localRemoteActions lastObject];
